...
router.post('/order-paid', async (req, res) => {
  ...
  // --- Step 2: Extract order details from Shopify payload ---
  ...
  // --- Step 3: Tell Shopify we received the webhook (must respond fast) ---
  // Shopify expects a 200 response within 5 seconds
  // So we respond immediately and process in background
  res.status(200).json({ received: true })

  // --- Step 4: Process allocation in background ---
  try {
    // Build the line items in the format our allocate endpoint expects
    ...
    // Call our own allocation logic directly
...
// ----------------------------------------------------------
// INTERNAL: Allocation logic (same as /orders/allocate but called directly)
// ----------------------------------------------------------
async function allocateOrder(order_id, line_items) {
  ...
  for (const item of line_items) {
    ...
    if (!variant_id || variant_id === 'null') {
      // Skip items with no variant (e.g. custom line items)
      continue
    }

    // Get fulfilling locations (HSR right now, future multi-location)
    ...
    if (!availablePieces || availablePieces.length < quantity) {
      ...
      // Tag the Shopify order for manual review
    ...
    for (const piece of availablePieces) {
      const { data: updated } = await supabase
        ...
        .eq('status', 'available') // Atomic lock
        .select()
        .single()

      if (!updated) {
        failures.push({ variant_id, jewelcode: piece.jewelcode, reason: 'race_condition' })
        continue
      }
      ...
      results.push({ jewelcode: piece.jewelcode, variant_id, order_id })
    }
  }
